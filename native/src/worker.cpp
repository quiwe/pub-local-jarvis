...
constexpr std::string_view kPerceptionPrompt = R"(Continuously analyze the current screen and system audio as AI Jarvis. Return ONLY one compact JSON object with exactly these fields: {"scene":"game|course|other","confidence":0.0,"barrage":"","course_note":"","course_title":"","course_interaction":"","capture_keyframe":false,"assistant_message":""}.
...
For any online course or lecture, regardless of subject, write course_note as one self-contained, review-worthy Chinese learning point supported by the current slide or speech. Adapt to the discipline: preserve concepts and terminology, claims and evidence, causes and consequences, dates and context, language rules and examples, procedures and cautions, code and behavior, formulas and conditions, case studies, experimental observations, or practical demonstrations whenever they are the actual teaching content. Capture the meaning and why it matters instead of mechanically copying sentences. Never describe the computer UI, windows, folders, files, player, teacher's physical actions, the user's activity, or your own observation process. Never output metadata or JSON inside course_note. If no new learning content is clear, use an empty string. course_title should identify the actual subject. course_interaction is a short, natural Chinese companion comment that helps the learner notice a key connection, recall relevant context, form a memory cue, or consider an application; it must not merely repeat course_note, must not quiz the user for an answer, and should be empty unless the current moment genuinely benefits from an interaction. Keep it under 50 Chinese characters. capture_keyframe should be true only for distinct material worth revisiting, such as an important slide, diagram, source passage, timeline, code example, formula, worked example, procedure, or demonstration. For other scenes, assistant_message must be a concise Chinese intervention only when there is an important error, risk, deadline, completion, or clearly useful next action; stay silent with an empty string for routine activity. Do not use Markdown and do not add text outside JSON.)";
}
