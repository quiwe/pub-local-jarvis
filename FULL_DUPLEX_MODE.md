...
普通主动文本有且只有一个生产入口：`duplex.decision(SPEAK)`。结构化感知即使返回旧版 `assistant_message` 或 `assistant_candidates` 字段，后端也必须丢弃，不能转换为 `assistant.message`。游戏弹幕和课程互动属于专用通道，不计入普通主动文本。
...
- 不复述播放视频、浏览文件、阅读网页、打开窗口和切换应用等日常画面。
- 只能观察并显示文本，不得声称可以点击、搜索、编辑、整理、打开或控制应用，也不得询问“需要我帮你……”或“要不要我……”。
- 证据模糊、文字看不清或只能猜测时必须选择 `LISTEN`。
...
4. 每条普通 `assistant.message` 的 `source` 必须为 `duplex`；注入带普通候选的结构化 `perception.completed` 不得产生气泡。
5. 相同情境持续 10 秒，不重复显示相同或近似提醒。
6. 情境发生实质变化后，允许模型对新的重要信息再次发言。
7. “需要我帮你”“要不要我”、日常画面复述、疑问句和不确定猜测被提示词与后端过滤共同阻断。
8. 暂停感知后，不再采集、推理或提醒，duplex 状态变为 inactive。
9. 恢复感知后，无需用户配置，duplex 自动恢复。
10. 没有 TTS 权重时仍能进入全双工文本模式，进程不尝试加载 TTS/Token2Wav。
11. 显存不足或 duplex 初始化失败时，APP 明确报错并回滚监控状态。

## 8. 兼容验收

全双工运行期间分别验证：

- 普通画面变化仍产生 `perception.completed` 供场景和记忆使用，但该事件不能生成普通气泡。
- 进入游戏仍切换游戏场景并产生去重弹幕。
- 游戏弹幕第二阶段复用当前游戏帧，只生成短候选并使用独立输出预算；生成前必须核对截图、HUD、动作和连续观察。具体游戏风格只来自当前陪伴方案，程序不得按游戏名称增加专属分支。模型结果缺字段、带尾随文本或生成失败时必须记录来源与兜底原因，不能静默退化。
- 播放课程仍创建课程、追加转写和知识点、请求关键帧并生成总结。
- 活动记忆仍按原置信度和时间窗口记录。
- 文本问答仍可提交和取消。
- 屏幕隐私、锁屏、显示器切换和 worker 退出不会遗留线程或临时媒体。

场景分类还必须输出显式证据：实时可交互游戏、全屏游戏媒体与普通网页内的游戏视频/直播分开，持续教学行为与普通网页、代码、文档和娱乐视频分开。实时游戏或几乎占满屏幕的游戏视频、直播和回放可进入 `game`。高置信游戏一帧进入，课程仍需两个连续一致样本；明确桌面、编辑器、普通网页等非游戏表面两帧退出，纯黑、模糊、加载或全字段为空的无证据结果需要四个连续样本才退出。游戏 HUD、游戏内菜单、比分板和回合结束画面作为持续游戏表面保留当前状态；课程确认前的首帧转写暂存，确认后补写，不能因防抖丢失课程开头。
